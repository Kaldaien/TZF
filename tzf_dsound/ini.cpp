...
{
  tzf::INI::File::Section section (name);
...
bool
Import_Section (tzf::INI::File::Section& section, wchar_t* buffer, int start, int end)
{
  int key = start;
  for (int k = key; k < end; k++) {
    if (k < end - 1 && buffer [k] == L'=') {
...
void
tzf::INI::File::import (std::wstring import_data)
{
...
void
tzf::INI::File::write (std::wstring fname)
{
  FILE*   fOut;
  errno_t ret;
...
const std::map <std::wstring, tzf::INI::File::Section>&
tzf::INI::File::get_sections (void)
{
  return sections;
}